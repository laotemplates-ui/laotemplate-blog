...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-paper/80 backdrop-blur-md border-b border-ink/10">
      <div className="max-w-5xl 2xl:max-w-7xl mx-auto px-4 py-5 2xl:py-6 flex items-center justify-between gap-6">
        <Link
          href="/"
        ...
        >
          <Image
            ...
            width={170}
            height={42}
            className="h-auto w-[170px] 2xl:w-[200px]"
            priority
          />
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-6 2xl:gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`text-sm 2xl:text-lg transition-colors ${
                isActive(link.href)
        ...
        <div className="flex items-center gap-3 2xl:gap-4 shrink-0">
          ...
          <a  href="https://laotemplate.com"
            className="hidden md:inline-block text-sm 2xl:text-base font-medium text-paper bg-gold px-4 py-2 2xl:px-6 2xl:py-3 rounded-full hover:bg-gold/90 transition-colors"
          >
            ກັບຄືນເວັບຫຼັກ
          </a>
